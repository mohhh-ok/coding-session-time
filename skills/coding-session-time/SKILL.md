...
3. Run `<cli> --json <flags>` and parse the array.
...
→ `<cli> --today --source all --json` → sum `seconds` → "2h32m today (148 prompts)."

User: "How much did I use Codex today?"
→ `<cli> --today --source codex --json` → sum `seconds` → "18m in Codex today (4 prompts)."

User: "How long did I spend on this project last week?"
→ `<cli> --last-week --here --json` → sum → "6h12m on this project last week."

User: "Top projects this month"
→ `<cli> --this-month --total --top 5 --json` → already aggregated by project, sort by `seconds` desc → list top 5.

User: "Daily breakdown for the last 30 days"
→ `<cli> --days 30 --json` → group by `date` → daily list.
...
- **Retention caveat.** Claude Code deletes `.jsonl` transcripts under `~/.claude/projects/` after `cleanupPeriodDays` (default 30 days). This tool can only report on transcripts that still exist on disk — deleted sessions cannot be reconstructed. If the user asks about a range older than ~30 days (e.g. "how many hours did I put into `myrepo` in April?" asked in July) and the result is empty or suspiciously low, do **not** conclude "you didn't work then". Say the underlying transcripts have likely been cleaned up, and suggest extending `cleanupPeriodDays` in `~/.claude/settings.json` (e.g. `3650`) to prevent it going forward. Never suggest `0` — due to a Claude Code bug, `0` disables transcript persistence entirely rather than meaning "keep forever". The same caveat applies to Codex (`~/.codex/sessions/`) when using `--source codex` / `--source all`.
